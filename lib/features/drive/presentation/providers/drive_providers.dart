...
/// MtprotoService is a FutureProvider because init() restores the session
/// asynchronously on startup. Also wires up TdlibService once authenticated.
final mtprotoServiceProvider = FutureProvider<MtprotoService>((ref) async {
  ...
  // Helper: call initTdlib if channel ID is configured
  Future<void> tryInitTdlib() async {
    final prefs     = await SharedPreferences.getInstance();
    ...
    if (channelId.isNotEmpty) {
      try {
        ...
        // Non-fatal — FileRepository falls back to Bot API path
      }
    }
  }

  // If already authenticated on startup, init TDLib immediately
  if (service.isAuthenticated) {
    await tryInitTdlib();
  }

  // Re-init TDLib whenever the user completes auth in a fresh session
