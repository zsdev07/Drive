...
class _C {
  ...
  // AES-IGE encryption/decryption using pointycastle (pure Dart, no NDK).
  static Uint8List aesIge(Uint8List data, Uint8List key, Uint8List iv, bool encrypt) {
    ...
    // Pad data to block boundary (AES = 16 bytes)
    final padded = data.length % 16 == 0
        ? data
        : Uint8List.fromList([...data, ...List.filled(16 - data.length % 16, 0)]);
